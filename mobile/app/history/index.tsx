...
export default function History() {
  ...
  return (
    <SafeAreaView style={s.page}>
      <Text style={s.title}>Historial de alertas</Text>
      {items.length ? (
        items.map((item) => (
          <Text key={item.id} style={s.item}>
            Alerta finalizada · {new Date(item.finishedAt).toLocaleString()}
          </Text>
        ))
      ) : (
        <Text style={s.copy}>Aún no hay alertas finalizadas.</Text>
      )}
    </SafeAreaView>
  );
}
