...
export default function Evidence() {
  ...
  const [message, setMessage] = useState(
    "Solo se admiten fotografías como evidencia.",
  );
  const add = async (camera: boolean) => {
    const r = await pickEvidence(camera);
    if (r.error) setMessage(r.error);
    else if (r.uri) {
      setUri(r.uri);
      setMessage("Evidencia almacenada localmente para esta demostración.");
    }
  };
  return (
    <SafeAreaView style={s.page}>
      <Text style={s.title}>Evidencia fotográfica</Text>
      <Pressable style={s.button} onPress={() => add(true)}>
        <Text style={s.buttonText}>Tomar fotografía</Text>
      </Pressable>
      <Pressable style={s.outline} onPress={() => add(false)}>
        <Text style={s.outlineText}>Seleccionar fotografía</Text>
      </Pressable>
...
const s = StyleSheet.create({
  ...
  outlineText: { color: "#9f1239", fontWeight: "800", textAlign: "center" },
  copy: { color: "#475569" },
