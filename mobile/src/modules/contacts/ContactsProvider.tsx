...
export type EmergencyContact = { id: string; name: string; phone: string; relationship: string };
type DirectoryPerson = Omit<EmergencyContact, 'relationship'>;
type ContactsContextValue = {
  contacts: EmergencyContact[];
  ready: boolean;
  addContact: (person: DirectoryPerson, relationship: string) => 'saved' | 'duplicate';
...
export function ContactsProvider({ children }: { children: ReactNode }) {
  ...
  const addContact: ContactsContextValue['addContact'] = (person, relationship) => {
    if (contacts.some((contact) => contact.id === person.id)) return 'duplicate';
    persist([...contacts, { ...person, relationship }]);
    return 'saved';
  };

  return <ContactsContext.Provider value={{ contacts, ready, addContact, updateContact: (id, relationship) => persist(contacts.map((contact) => contact.id === id ? { ...contact, relationship } : contact)), removeContact: (id) => persist(contacts.filter((contact) => contact.id !== id)) }}>{children}</ContactsContext.Provider>;
}
